    ...
    def train_and_evaluate_agent(self, epochs, target_update, batch_size):
        ...
        for i_episode in tqdm(range(epochs),desc='Training'):
            ...
            while not done:
                ...
                means_std_a_ep = self.agent_a.optimize(states, actions_a, new_states, reward, non_final)
                means_std_b_ep = self.agent_b.optimize(states, actions_b, new_states, reward, non_final)
            ...
            # for i in range(self.agent_a.number_heads):
            #     if means_std_a_ep[i,0] >0:
            #         means_a[i].append(means_std_a_ep[i,0].detach().numpy())
            #         std_a[i].append(means_std_a_ep[i,1].detach().numpy())
            #     if means_std_b_ep[i,0]> 0:
            #         means_b[i].append(means_std_b_ep[i,0].detach().numpy())
            #         std_b[i].append(means_std_b_ep[i,1].detach().numpy())
        ...
        agentType = 'PNN-DQN'+self.agent_a.agent_type_loss+self.agent_a.agent_type_train+self.agent_a.agent_type_eval#type(self.agent_a).__name__
